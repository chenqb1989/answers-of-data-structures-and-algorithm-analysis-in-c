...
int union_list(t_list list1, t_list list2, t_list ulist)
{
    ptr_to_node pos1 = list1->next;
    ptr_to_node pos2 = list2->next;
    ptr_to_node pos_u = ulist;
    while (pos1 != NULL || pos2 != NULL)
    {
        if (pos1 == NULL)
        {
            int elem2 = pos2->element;
            if (elem2 != pos_u->element) // to deal with duplicate number
            {
                ptr_to_node tmp_cell = (ptr_to_node)malloc(sizeof(struct t_node));
                tmp_cell->element = elem2;
                tmp_cell->next = pos_u->next;
                pos_u->next = tmp_cell;
                pos2 = pos2->next;
            }
            else
...
int main()
{
    ...
    const int len2 = 8;
    const int a2[8] = {2, 4, 7, 10, 15, 20, 20, 25};
