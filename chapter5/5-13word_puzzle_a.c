/*
note:
1. for simplicity, just find word in four directions: left, right, up, down
*/
#include <stdlib.h>
#include <stdio.h>
#include <time.h>
...
hash_table create_hash_dict(char *str[], int str_num, int table_size)
{
...
int word_puzzle(char str[][4], int row, int column, hash_table h)
{
    ...
    for (int r = 0; r < row; r++)
    {
        for (int c = 0; c < column; c++)
        {
            ...
            for (int l = 0; c + l < column; l++)
            {
                ...
                s[l + 1] = '\0';
                if (h->the_cells[find_square(s, h)].info == legitimate)
...
int main()
{
    ...
    int number = 6;
    char *s[] = {"it", "ih", "fowt", "this", "haa", "ithd"};
    
    hash_table dict = create_hash_dict(s, number, 101);
    print_hash_table(dict);
